# Table of contents

* [Gadaga APIS](README.md)

## OAuth Login APIS \(10000\) <a id="oauth-login-apis"></a>

* [User Login API](oauth-login-apis/user-login-api.md)
* [Refresh Token API](oauth-login-apis/refresh-token-api.md)

## User APIS \(10010\) <a id="user-modules"></a>

* [User Command Resource](user-modules/user-command-resource/README.md)
  * [User Sign Up API](user-modules/user-command-resource/user-sign-up-api.md)
  * [Change Email API](user-modules/user-command-resource/change-email-api.md)
  * [Change Nickname API](user-modules/user-command-resource/change-nickname-api.md)
  * [Change Password API](user-modules/user-command-resource/change-password-api.md)
  * [Delete User API](user-modules/user-command-resource/delete-user-api.md)
* [User Query Resource](user-modules/user-query-resource/README.md)
  * [Get User API](user-modules/user-query-resource/get-user-api.md)
  * [Check Password API](user-modules/user-query-resource/check-password-api.md)
  * [Find Email API](user-modules/user-query-resource/find-email-api.md)
  * [Find Password API](user-modules/user-query-resource/find-password-api.md)
  * [Validation Check API](user-modules/user-query-resource/validation-check-api.md)
* [Store Category Query Resource](user-modules/store-category-query-resource/README.md)
  * [Get Store Category List API](user-modules/store-category-query-resource/get-store-category-list-api.md)
* [Store Query Resource](user-modules/store-query-resource/README.md)
  * [Get Store List By Location API](user-modules/store-query-resource/get-store-list-by-location-api.md)
  * [Get Store List By Route API](user-modules/store-query-resource/get-store-list-by-route-api.md)
  * [Get Store By Id](user-modules/store-query-resource/get-store.md)
* [Review Command Resource](user-modules/untitled-2/README.md)
  * [Create Review API](user-modules/untitled-2/create-review-api.md)
* [Untitled](user-modules/untitled-3.md)
* [Untitled](user-modules/untitled-4.md)
* [Untitled](user-modules/untitled-6.md)
* [Untitled](user-modules/untitled-5.md)

## Owner APIS \(10020\) <a id="owner-apis"></a>

* [OAuth](owner-apis/oauth.md)

